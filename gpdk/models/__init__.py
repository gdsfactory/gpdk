...
def get_models() -> dict[str, Callable[..., sax.SDict]]:
    ...
    for name, func in list(globals().items()):
        ...
        try:
            ...
            continue
        # Check for sax.SDict return type: either the evaluated alias object
        # (`-> sax.SDict` without `from __future__ import annotations`) or a
        # string annotation containing "sdict" (case-insensitive).
        return_anno = sig.return_annotation
        if return_anno == sax.SDict or "sdict" in str(return_anno).lower():
            models[name] = func
    return models
